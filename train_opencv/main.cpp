...
int main(int argc,char** argv)
{
    ...
    int nLine = 0;
    string buf;
    ifstream svm_data( "/work/shintaogas/code/train_opencv/train_opencv/num/SVM_DATA.txt" );//训练样本图片的路径都写在这个txt文件中，使用bat批处理文件可以得到这个txt文件
    unsigned long n;
    ...
    //保存训练好的分类器
    svm.save( "/work/shintaogas/code/train_opencv/train_opencv/HOG_SVM_DATA.xml" );
    
    //检测样本
    IplImage *test;
    char result[512];
    vector<string> img_tst_path;
    ifstream img_tst( "/work/shintaogas/code/train_opencv/train_opencv/train_test.txt" );  //加载需要预测的图片集合，这个文本里存放的是图片全路径，不要标签
    while( img_tst )
    {
    ...
    ofstream predict_txt( "/work/shintaogas/code/train_opencv/train_opencv/SVM_PREDICT.txt" );//把预测结果存储在这个文本中
